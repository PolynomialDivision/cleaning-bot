...
pub enum DomainEvent {
    ...
    RotationQueueSet   { group_id: GroupId, queue: Vec<PersonId> },
...
mod tests {
    ...
    fn old_slot_assigned_events_without_the_new_audit_fields_still_deserialize() {
        ...
        let event: DomainEvent = serde_json::from_str(json).expect("old event shape must still parse");
        match event {
            DomainEvent::SlotAssigned { source, actor_id, previous_person_id, person_id, .. } => {
                assert_eq!(source, crate::domain::AssignmentSource::Manual);
                assert_eq!(actor_id, None, "missing field must default to None, not error");

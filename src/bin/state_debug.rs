...
fn main() -> anyhow::Result<()> {
    ...
    let state: State = serde_json::from_str(&raw).unwrap_or_else(|e| panic!("Parse error: {e}"));
    ...
    // ── Metadata ──────────────────────────────────────────────────────────────
    if let Some(t) = &state.created_at {
        println!("Created:       {t}");
    }
    if let Some(t) = &state.last_modified {
    ...
    for c in sorted.iter().take(8) {
        ...
        let group = state
            ...
            .unwrap_or("?");
        println!(
            "  {:04}-W{:02}  {group:<20}  by {who}",
            c.iso_year, c.iso_week
        );
    }
    if sorted.len() > 8 {
        println!("  … ({} more)", sorted.len() - 8);
    }
    println!();

    // ── Future schedule ───────────────────────────────────────────────────────
    let (cur_y, cur_w) = current_iso_week();
    let mut future: Vec<_> = state
        .slot_assignments
        .iter()
    ...
    for a in future.iter().take(10) {
        ...
        let slot_name = state
            ...
            .map(|s| format!(" / {}", s.name))
            .unwrap_or_default();
        let current_tag = if a.iso_year == cur_y && a.iso_week == cur_w {
            " ← THIS WEEK"
        } else {
            ""
        };
        println!(
            "  {:04}-W{:02}  {group}{slot_name:<28} → {who}{current_tag}",
            a.iso_year, a.iso_week
        );
    }
    if future.len() > 10 {
        println!("  … ({} more)", future.len() - 10);
    }
    println!();
    ...
    if !state.absences.is_empty() {
        ...
        for a in &state.absences {
            ...
            let group = state
                ...
                .unwrap_or("?");
            println!(
                "  {who} in {group}: from {:04}-W{:02} for {} weeks",
    ...
    // ── Stubs / orphans ───────────────────────────────────────────────────────
    let grouped: HashSet<&str> = state
        .cleaning_groups
        .iter()
        .flat_map(|g| g.member_ids.iter().map(String::as_str))
        .collect();
    let stubs: Vec<_> = state
        .persons
        .iter()
    ...
    if !stubs.is_empty() {
        ...
        for p in &stubs {
            ...
            println!(
                "  {} ({}) — {reason}",
                p.display_name,
                p.matrix_id.as_deref().unwrap_or("no mxid")
            );
        }
        println!();
    }
    ...
    let valid_gids: HashSet<&str> = state
        .cleaning_groups
        .iter()
        .map(|g| g.id.as_str())
        .collect();
    ...
    for g in &state.cleaning_groups {
        for pid in &g.member_ids {
            if !valid_pids.contains(pid.as_str()) {
        ...
        for pid in &g.member_ids {
            if !ms.insert(pid.as_str()) {
                warnings.push(format!("Group «{}» has duplicate member {pid}", g.name));
            }
        }
    }

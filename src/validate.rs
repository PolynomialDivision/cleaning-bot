...
pub fn validate_state(state: &State) -> ValidationReport {
    ...
    let valid_group_ids: HashSet<&str> = state
        .cleaning_groups
        .iter()
        .map(|g| g.id.as_str())
        .collect();
    ...
    for g in &state.cleaning_groups {
        for pid in &g.member_ids {
            if !valid_person_ids.contains(pid.as_str()) {
                errors.push(format!(
                    "Group «{}» references unknown PersonId: {}",
                    g.name, pid
                ));
            }
        }
        ...
        for pid in &g.member_ids {
            if !ms.insert(pid.as_str()) {
                warnings.push(format!(
                    "Group «{}» contains duplicate member ID: {}",
                    g.name, pid
                ));
            }
        }
    ...
    for (i, c) in state.completions.iter().enumerate() {
        if c.group_id.is_empty() {
            warnings.push(format!(
                "Completion #{i} has no group_id (migration incomplete?)"
            ));
        } else if !valid_group_ids.contains(c.group_id.as_str()) {
            warnings.push(format!(
                "Completion #{i} references unknown GroupId: {}",
                c.group_id
            ));
        }
        if c.completed_by_id.is_empty() {
            warnings.push(format!(
                "Completion #{i} has no completed_by_id (migration incomplete?)"
            ));
        } else if !valid_person_ids.contains(c.completed_by_id.as_str()) {
            warnings.push(format!(
                "Completion #{i} references unknown PersonId: {}",
                c.completed_by_id
            ));
        }
    }
    ...
    for (i, a) in state.absences.iter().enumerate() {
        if a.person_id.is_empty() {
            warnings.push(format!(
                "Absence #{i} has no person_id (migration incomplete?)"
            ));
        } else if !valid_person_ids.contains(a.person_id.as_str()) {
            warnings.push(format!(
                "Absence #{i} references unknown PersonId: {}",
                a.person_id
            ));
        }
        if a.group_id.is_empty() {
            warnings.push(format!(
                "Absence #{i} has no group_id (migration incomplete?)"
            ));
        } else if !valid_group_ids.contains(a.group_id.as_str()) {
            warnings.push(format!(
                "Absence #{i} references unknown GroupId: {}",
                a.group_id
            ));
        }
    }
    ...
    for s in &state.swap_requests {
        if s.group_id.is_empty() {
            warnings.push(format!(
                "Swap #{} has no group_id (migration incomplete?)",
                s.id
            ));
        } else if !valid_group_ids.contains(s.group_id.as_str()) {
            warnings.push(format!(
                "Swap #{} references unknown GroupId: {}",
                s.id, s.group_id
            ));
        }
    }
    ...
    // ── 7. CalendarTokens ─────────────────────────────────────────────────────
    {
        ...
        for ct in &state.calendar_tokens {
            ...
            if !valid_person_ids.contains(ct.person_id.as_str()) {
                warnings.push(format!(
                    "CalendarToken {} references unknown PersonId: {}",
                    ct.id, ct.person_id
                ));
            }
            if !ct.revoked {
        ...
        for (pid, count) in active_per_person {
            if count > 1 {
                warnings.push(format!(
                    "Person {pid} has {count} active calendar tokens (expected 1)"
                ));
            }
        }
    }
    ...
    let assigned_persons: HashSet<&str> = state
        .cleaning_groups
        .iter()
    ...
    for p in &state.persons {
        if p.active && !assigned_persons.contains(p.id.as_str()) {
            warnings.push(format!(
                "Person «{}» is active but not in any cleaning group",
                p.display_name
            ));
        }
    }
